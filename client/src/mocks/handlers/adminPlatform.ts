...
import fakeProfileData from "../fake/profiel";

import { rest } from "msw";
import { token } from "../fake/token";

const PLATFORM_AUTH_URL =
    "https://login.scoutsengidsenvlaanderen.be/auth/realms/scouts/protocol/openid-connect/token";
...
export const adminPlatformHandlers = [
    ...
    rest.get(PLATFORM_API_URL + "/lid/profiel", (_, res, ctx) => {
        return res(ctx.status(200), ctx.json(fakeProfileData));
    })
];
