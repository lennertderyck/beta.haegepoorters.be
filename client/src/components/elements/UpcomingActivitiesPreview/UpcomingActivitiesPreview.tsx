...
const UpcomingActivitiesPreview: FC<Props> = () => {
    ...
    const handleActivityClick = (groupId: string, eventId: string) => {
        if (focusedActivity === groupId)
            navigate(`/haegeprekerke/${groupId}/activities/${eventId}`);
        else setFocusedActivity(groupId);
    };

    const timelines = query.data?.timelineForGroups.filter(
        (timeline: any) => !!timeline.upcomingActivity
    );
                    ...
                    >
                        <div>
                            <span className="font-bold text-3xl text-gray-400">
                                <Date format="DD">{event.startDate}</Date>
                            </span>
                            <div className="text-center font-serif uppercase text-gray-400 leading-[.5rem]">
                                <Date format="MMM">{event.startDate}</Date>
                            </div>
                        </div>
                        <div>
                            ...
                            {!!event.body && (
                                <ExpansionPane
                                    active={focusedActivity === group.abbr}
                                >
